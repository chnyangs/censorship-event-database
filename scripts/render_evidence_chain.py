...
EVIDENCE_CHAIN_MARKER = ".evidence_chains_output"
...
def _event_status_by_slug() -> dict[str, str]:
    statuses: dict[str, str] = {}
    for path in sorted(EVENTS_DIR.glob("*.yaml")):
        event = yaml.safe_load(path.read_text())
        if isinstance(event, dict) and isinstance(event.get("id"), str):
            statuses[event["id"]] = str(event.get("status") or "")
    return statuses


def prepare_output_dir(out_dir: pathlib.Path) -> pathlib.Path:
    raw_path = out_dir.expanduser()
    resolved = raw_path.resolve()
    default_out = DEFAULT_OUT_DIR.resolve()
    if resolved.exists():
        marker = resolved / EVIDENCE_CHAIN_MARKER
        if resolved != default_out and not marker.exists():
            raise SystemExit(
                f"[render_evidence_chain] refusing existing unmarked --output-dir: {resolved}"
            )
    resolved.mkdir(parents=True, exist_ok=True)
    (resolved / EVIDENCE_CHAIN_MARKER).write_text(
        "Generated by scripts/render_evidence_chain.py; stale chains may be removed.\n"
    )
    return resolved
...
def render_event_evidence_chain(
    ...
    if related:
        ...
        out.append("")
        for r in related:
            r_status = statuses.get(str(r))
            if r_status == "admitted":
                out.append(f"- [`{r}`](./{r}.md)")
            else:
                label = f"`{r}`"
                if r_status:
                    label += f" ({r_status}; no rendered admitted-chain link)"
                else:
                    label += " (not found; no rendered admitted-chain link)"
                out.append(f"- {label}")
        out.append("")

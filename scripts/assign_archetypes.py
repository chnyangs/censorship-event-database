...
Additional derived fields per event:
  ...
  - trigger_is_action        true when trigger.type is `corporate_policy_change`.
      Useful qualifier on `latency_regime: synchronous` rows: when the
      trigger IS itself the action (e.g. Tether issuing a blacklist), t=0
      is definitional, not a measured reaction speed. Paper claims about
      cascade speed should report (synchronous AND NOT trigger_is_action)
      as the "true rapid cascade" subset.
...
def render_report(
    ...
    # synchronous cases: separate "trigger IS action" from "real rapid cascade"
    sync = [r for r in rows if r["latency_regime"] == "synchronous"]
    if sync:
        t_is_a = [r for r in sync if r.get("trigger_is_action")]
        real_rapid = [r for r in sync if not r.get("trigger_is_action")]
        notes.append(
            f"- `synchronous` (≤1h) bucket has {len(sync)} events, but **{len(t_is_a)} are "
            f"trigger≡action** (trigger.type = corporate_policy_change — the trigger *is* "
            f"the observed action; t=0 is definitional, not a measured reaction speed): " +
            ", ".join(f"`{r['event_id']}`" for r in t_is_a) + ". "
            f"The remaining **{len(real_rapid)} events are true same-hour cascades** "
            "(external trigger with ≤1h observed reaction)."
            " Paper claims about cascade reaction speed should use the `not trigger_is_action` "
            "subset as the numerator; the `trigger_is_action` subset belongs in a separate "
            "discussion of how issuer-policy events collapse the trigger/action boundary."
        )

    # lagged cases: the long tail
    lagged = [r for r in rows if r["latency_regime"] == "lagged"]
    if lagged:
        notes.append(
            f"- {len(lagged)} event(s) in `lagged` (>30d). These are the 'issuer-clock ≠ "
            "regulator-clock' tail — they support the empirical claim that issuer compliance "
            "operates on an independent schedule. Worth enumerating: " +

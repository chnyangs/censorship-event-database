...
def freeze(candidate_dir, out):
    ...
    manifest = {'protocol_version': 'official-public-disclosure-v1', 'status': 'frozen_before_collection_machine_not_human_adjudicated',
                ...
                'enumeration_rule': 'Kraken documented WordPress posts REST route if discoverable, chronological100posts/page, maximum3pages/window with page-count checks. Otherwise official indices plus robots-declared sitemap traversal, max8documents/operator; prioritize sitemap URLs containing blog/support/announcement, then lexical URL order. Fetch up to16 lexical URL/anchor matches sorted lexicographically. Sitemap lastmod is never publication time.',
...
def sitemap_priority(url):
    return (not any(term in url.casefold() for term in ('blog','support','announcement')),url)
...
def main():
    ...
    result=freeze(args.candidate_dir,args.out_dir) if args.command=='freeze' else run(args.protocol,args.out_dir)

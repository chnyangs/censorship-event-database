...
def validate_result(method: str, result):
    if method == "eth_chainId":
        ...
        for key in ("number", "timestamp"):
            if not isinstance(result[key], str) or not re.fullmatch(r"0x[0-9a-fA-F]+", result[key]):
                raise ValueError(f"Malformed block {key}")
        if not isinstance(result["hash"], str) or not re.fullmatch(r"0x[0-9a-fA-F]{64}", result["hash"]):
            raise ValueError("Malformed block hash")
        # Keep invalid/unrepresentable timestamps inside the captured gap path.
        try:
            datetime.fromtimestamp(int(result["timestamp"], 16), timezone.utc)
        except (ValueError, OverflowError, OSError) as exc:
            raise ValueError("Block timestamp is out of range") from exc
...
def decode_target_logs(logs: list, contract: dict, target: str = TARGET, pilot_block: int = PILOT_BLOCK) -> list:
    ...
    for log in logs:
        ...
        if not isinstance(log.get("address"), str) or log["address"].lower() != contract["address"]:
            raise ValueError("Response contains a log for another contract")
        topics = log.get("topics")
        if not isinstance(topics, list) or not topics or any(not isinstance(topic, str) or not re.fullmatch(r"0x[0-9a-fA-F]{64}", topic) for topic in topics):
            raise ValueError("Malformed event topics")
        if topics[0].lower() not in contract["topics"] or log.get("removed") is not False:
        ...
        if contract["indexed_address"] and len(topics) != 2:
            raise ValueError("Indexed event address topic missing")
        if not contract["indexed_address"] and len(topics) != 1:
            raise ValueError("Unexpected non-indexed event topic")
        data = log.get("data")
        if not contract["indexed_address"] and (not isinstance(data, str) or not re.fullmatch(r"0x[0-9a-fA-F]{64}", data)):
            raise ValueError("Malformed event data word")
        location = topics[1][2:] if contract["indexed_address"] else data[2:]
        if len(location) != 64 or location[:24] != "0" * 24:
...
def run_pilot(out: Path, panel_manifest: Path, transport=http_once, pilot_block: int = PILOT_BLOCK,
    ...
    manifest = {
        ...
        "selection": "Tornado Cash training case; supplied feasibility slice on the legacy-coded trigger date, not an independently verified sanctions trigger or a verified transition boundary. Known case selection is outcome-informed training, never evaluation sampling.",
    ...
    for provider in providers:
        ...
        for query in queries:
            ...
            if query["id"] == "historical_block" and result["status"] == "valid_rpc_response":
                ...
                historical_block_ok = result["status"] == "valid_rpc_response"
            if query["id"] not in {"chain_id", "historical_block"} and not query["id"].endswith("_latest"):
                result["historical_block_dependency_valid"] = historical_block_ok
                if not historical_block_ok:
                    result["interpretation_limit"] = "Transport diagnostic only; block identity/time dependency unavailable or invalid"

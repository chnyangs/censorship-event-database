...
Generated: `2026-04-24T09:42:41Z` · scanner `scripts/scan_operator_census.py` · candidates `sources/operator_census/candidates.yaml`.
...
| flashbots | `flashbots/builder` | block_builder_deprecated | ok | 0 | 0 | 0 | 0 | 0 | 0 | — | — |
| metamask | `MetaMask/eth-phishing-detect` | wallet_blocklist | ok | 1 | 204341 | 0 | 0 | 8 | 1450 | — | — |
| trust_wallet | `trustwallet/assets` | wallet_token_index | ok | 0 | 0 | 0 | 0 | 0 | 0 | — | — |
...
- **Classification is keyword-based**, not semantic. A commit that touches `ofacblacklist.go` with the subject *"refactor"* would land in `list_maintenance`, but one that touches the same file with a semantically-sanctions-related subject using different words could slip to `other`. The raw JSON emits the full (subject, path, +/-) tuple so an auditor can re-classify.

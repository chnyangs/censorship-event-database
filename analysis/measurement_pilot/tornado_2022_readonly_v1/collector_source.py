...
def decode_target_logs(logs: list, contract: dict, target: str = TARGET) -> list:
    selected = []
    ...
    for log in logs:
        ...
        location = log["topics"][1][2:] if contract["indexed_address"] and len(log["topics"]) > 1 else log.get("data", "")[2:66]
        if len(location) != 64:
...
def run_pilot(out: Path, panel_manifest: Path, transport=http_once) -> dict:
    if out.exists():
    ...
    manifest = {
        ...
        "selection": "Known historical Tornado Cash positive-control example from existing YAML; intentionally outcome-informed training, never evaluation sampling",
    ...
    for provider, endpoint in ENDPOINTS.items():
        ...
        for query in queries:
            ...
            if query["id"].endswith("_logs") and result["status"] == "valid_rpc_response":
                try:
                    result["target_logs"] = decode_target_logs(result["result"], CONTRACTS[query["id"].split("_")[0]])
...
def main() -> int:
    ...
    print(json.dumps(run_pilot(args.out_dir, args.panel_manifest), indent=2))
    return 0

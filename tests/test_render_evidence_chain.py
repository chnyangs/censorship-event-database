...
def test_related_event_statuses_can_be_precomputed(monkeypatch):
    def fail_status_scan():
        raise AssertionError("status map should be reused")

    monkeypatch.setattr(render_evidence_chain, "_event_status_by_slug", fail_status_scan)
    event = {
        "id": "subject",
        "status": "admitted",
        "schema_version": "0.2.0",
    ...
    rendered = render_evidence_chain.render_event_evidence_chain(
        event,
        {},
        event_statuses={"admitted-related": "admitted", "draft-related": "draft"},
    )

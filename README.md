...
1. **Motivation** — prior measurement work sees layers individually; a coverage-denominator-disciplined catalog reveals *where public operator behavior is actually observable* and *where it is not*. One public RPC operator's source-control history (Flashbots' `rpc-endpoint`) is used as a worked exemplar of an under-audited substrate. A reproducible 8-repo census ([`analysis/operator_census/README.md`](analysis/operator_census/README.md)) confirms this channel is structurally narrow: of 8 surveyed operator repositories (Flashbots relay / builder / rbuilder, MetaMask eth-phishing-detect, Trust Wallet, Uniswap token-list schema, Ethereum-Lists tokens, and the rpc-endpoint baseline), **exactly one repo (`flashbots/rpc-endpoint`) ships a named OFAC filter file in public git, and exactly one commit across the entire surveyed corpus carries OFAC keywords in its subject** (Flashbots PR #90, the canonical 2022-08-08 update). Six surveyed repos ship no public compliance substrate at all; MetaMask's heavy phishing-blocklist activity carries zero OFAC-keyword commits under subject-only classification.
...
**Worked exemplars**: (a) the Flashbots `rpc-endpoint` bidirectional case (n=1 operator, archived body-hash-verifiable forward + reverse); (b) upper-layer concentration of observed reactions under coverage-matched conditioning (C1); (c) single-layer dominance in the admitted corpus (35 / 53, C2); (d) observability gap on L0 / L1 / L3 as a measured feature of the public evidentiary substrate, not an artifact of curation.

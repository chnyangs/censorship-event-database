...
**A coverage-denominator-disciplined catalog of publicly documented crypto censorship events, instrumented to ask what operators actually do — with a worked existence proof for operator filter-list maintenance in public git, and a reusable admission protocol for enforcement-observability measurement.**
...
2. **A worked mechanism exemplar (n=1 operator) — git-observable filter-list maintenance on one public RPC repo.** Flashbots' [rpc-endpoint::ofacblacklist.go](https://github.com/flashbots/rpc-endpoint) adds OFAC-listed Tornado pool addresses via commit `92ab6b1f` **2h 50m** after the 2022-08-08 SDN (PR #90 "update ofac black list"), and the entire 132-address map is deleted in commit `1e9c29c` eleven days after OFAC's 2025-03-21 delisting (PR #173). Same infrastructure, bidirectional, primary-corporate, minute-precise. This is an **existence proof** — one operator, one repo, the forward and reverse editions of a single target list — **not** a population claim about operator behavior, nor a claim that filter-list maintenance is the dominant crypto-censorship mechanism. The multi-repo census currently supports a narrow observability-gap result, not generalization from the exemplar to a population-level channel. See [analysis/anchor_gap_fill_log.md §4](analysis/anchor_gap_fill_log.md) for the reproducibility trail.
...
1. **Motivation** — prior measurement work sees layers individually; a coverage-denominator-disciplined catalog reveals *where public operator behavior is actually observable* and *where it is not*. One public RPC operator's source-control history (Flashbots' `rpc-endpoint`) is used as a worked exemplar of an under-audited substrate. A reproducible 8-repo census ([`analysis/operator_census/README.md`](analysis/operator_census/README.md)) confirms this channel is structurally narrow under the configured current-or-historical path patterns: of 8 surveyed operator repositories (Flashbots relay / builder / rbuilder, MetaMask eth-phishing-detect, Trust Wallet, Uniswap token-list schema, Ethereum-Lists tokens, and the rpc-endpoint baseline), **two repos expose OFAC-named current or historical files, but exactly one repo (`flashbots/rpc-endpoint`) has an OFAC-keyword reaction commit tied to an admitted event** (Flashbots PR #90, the canonical 2022-08-08 update). The other matched list substrates are deprecated or non-OFAC list-maintenance streams under subject-only classification.
2. **Methodology contribution** — the six-layer admission protocol, coverage-matched conditional rates (a direct port of OONI-style denominator discipline to crypto-censorship measurement), precision-aware latency filtering, attribution discipline, and fail-closed paper-table generation as a reusable measurement framework.
3. **Worked mechanism exemplar — Flashbots rpc-endpoint (n=1 operator)** — primary case study: `ofacblacklist.go` grows by +Tornado-pool addresses in PR #90, **2h 50m** after the 2022-08-08 SDN (direct attribution via commit title "update ofac black list"), and the entire 132-entry map is deleted in PR #173, eleven days after OFAC's 2025-03-21 delisting (plausible attribution; PR title frames as operational cleanup, not policy). Bidirectional, minute-precise, primary-corporate — on **one** operator's repo. We explicitly decline to generalize from this exemplar to a population-level "operator-layer channel" claim (see §6 below).
...
6. **Limitations and scope boundary** — sampling frame is admissibility-bounded, English-indexable, and US/EU-trigger-dominant, not a population sample; survivorship bias in the evidence substrate is structural, not fixable by adding events; hour-precision latency is supportable only on named events, not as a distribution; the Flashbots case is **one operator** and the 8-repo census confirms no other surveyed operator exposes an OFAC-keyed filter file in public source control. Scope expansion to non-US relay/builder implementations, SUAPP/SUAVE, and additional wallet/frontend repos is named v0.2 open work in [`analysis/operator_census/README.md`](analysis/operator_census/README.md).
...
**Primary contribution (framework-level)**: a forkable admission protocol and paper-table generator that make cross-layer enforcement observability measurable with coverage-matched conditional rates, attribution discipline, and body-hash-anchored evidence. Layer 2 in the Thesis.

**Worked exemplars**: (a) the Flashbots `rpc-endpoint` bidirectional case (n=1 operator, archived body-hash-verifiable forward + reverse); (b) upper-layer concentration of observed reactions under coverage-matched conditioning (C1); (c) the descriptive archetype table, parked as a central claim until `observation_kind` IRR clears; (d) observability gap on L0 / L1 / L3 as a measured feature of the public evidentiary substrate, not an artifact of curation.

**What this paper explicitly does not claim**: that operator-layer filter-list maintenance is the dominant crypto-censorship mechanism in the wild, that the six-layer split captures the population, that the rate figures estimate a population prevalence, or that the Tornado forward+reverse case alone establishes a generalizable operator channel. See [docs/paper_claims.md §0 "What this paper is NOT"](docs/paper_claims.md).
